			...
			<div class="body"> 
                ...
                <div class="input-group mb-3">
                    ...
                    <div class="input-group-append">
                        <button class="btn btn-success" id="simpan" type="button">Simpan Klasis</button>
                    </div>
                </div>
...
$(document).ready(function(){
    $("#simpan").click(function(){
        ...
        if(judul=="") {
            alert('Silakan isi nama Klasis, terlebih dahulu.');
        }else if(isi==""){
            alert('Silakan isi keterangan dan gambar Klasis, terlebih dahulu.');
        }else{
			$.ajax({
                type : "POST",
                url  : "<?= base_url('admin/ubah_post_klasis')?>",
                data : {judul:judul,isi:isi,id:id},
                beforeSend: function(){
					$('#popup_loading').show();
            	}, success: function(){
					window.location.href = "<?= base_url('admin/klasis')?>";
                    console.log('berhasil diubah');
                }, error:function(){
					alert('Gagal mengubah data Klasis, silahkan ulangi kembali.');
					$('#popup_loading').hide();
                }
            });
		}  
